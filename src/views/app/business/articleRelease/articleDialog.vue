<template>
  <el-dialog :title="title+'消息'"
             :visible="showNewsInfo"
             :before-close="handleClose"
             width="800px"
             :close-on-click-modal="false"
             append-to-body>
    <el-form size='mini' ref="form" :model="form" :rules="rules" label-width="90px">
<!--      <el-form-item label="类型" prop="type">-->
<!--        <el-select v-model="form.type" placeholder="请选择类型" clearable>-->
<!--          <el-option label="新闻资讯" value="2"></el-option>-->
<!--        </el-select>-->
<!--      </el-form-item>-->
<!--      <el-form-item label="序号" prop="order">-->
<!--        <el-input v-model="form.order" placeholder="请输入序号" />-->
<!--      </el-form-item>-->
      <el-form-item label="标题" prop="title" >
        <el-input v-model="form.title" placeholder="请输入标题" />
      </el-form-item>
      <el-form-item label="描述" prop="description">
        <el-input v-model="form.description" placeholder="请输入序号" />
      </el-form-item>
      <el-form-item label="图片">
        <el-upload
          class="upload-demo"
          :action="uploadImgUrl"
          :show-file-list="false"
          :headers="headers"
          name="file"
          :on-success="handleUploadSuccess"
          :before-upload="handleBeforeUpload"
          :on-error="handleUploadError">
          <el-button size="small" type="primary">点击上传</el-button>
          <div slot="tip" class="el-upload__tip" style="color:#f00">建议尺寸750*300，图片大小小于3M</div>
        </el-upload>
        <el-image
          v-show="form.banner"
          style="width: 100px; height: 100px"
          :src="form.banner"
          :preview-src-list="srcList">
        </el-image>
      </el-form-item>
      <el-form-item label="内容" prop="content">
<!--        <el-input v-model="form.content" type="textarea" :autosize="{ minRows: 2, maxRows: 6}"  placeholder="请输入内容" />-->
        <editor v-model="form.content" :min-height="192"/>
      </el-form-item>
    </el-form>
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" @click="submitForm">确 定</el-button>
      <el-button @click="cancel">取 消</el-button>
    </div>
  </el-dialog>
</template>

<script>
import Editor from '@/components/Editor';
import {addArticle,getArticleDetail} from '@/api/business/articleRelease'
import { getToken } from "@/utils/auth";
export default {
  name: 'newsDialog',
  components: {
    Editor
  },
  props:{
    showNewsInfo:Boolean,
    isEdit:Boolean,
    title:String,
    rowId:[String,Number],
  },
  data() {
    return {
      form:{
        type:2,
        title:'',
        content:'',
        banner:'',
        description:''
      },
      rules:{},
      uploadImgUrl: process.env.VUE_APP_BASE_API + "/file/upload", // 上传的图片服务器地址
      headers: {
        Authorization: "Bearer " + getToken(),
      },
      loading:null,
      srcList:[]
    }
  },
  watch:{
    isEdit(val){
      if(val){
        this.getArticleInfo(this.rowId);
      }
    }
  },
  methods: {
    getArticleInfo(articleId){
      let id={
        id:articleId
      }
      getArticleDetail(id).then((response) => {
        this.form=response.data;
        this.srcList=[]
        this.srcList.push(this.form.banner);
      });
    },
    submitForm(){
      this.$refs["form"].validate((valid) => {
        if (valid) {
          console.log(this.form)
          if(this.form.banner==''){
            this.msgError("上传图片不能为空！");
            return false;
          }
          if (!this.isEdit) {
            addArticle(this.form).then((response) => {
              this.msgSuccess("新增成功");
              this.$emit('reload');
              this.handleClose();
            });
          } else {
            addArticle(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.$emit('reload');
              this.handleClose();
            });
          }
        }
      });
    },
    handleUploadSuccess(res) {
      console.log(res)
      this.srcList=[]
      this.form.banner=res.data.url;
      this.srcList.push(res.data.url)
      this.$message({
        type: "success",
        message: "上传成功！",
      });
      this.loading.close();
    },
    handleBeforeUpload(file) {
      const typeArr = ['image/png', 'image/gif', 'image/jpeg', 'image/jpg']
      const isJPG = typeArr.indexOf(file.type) !== -1
      const isLT3M= file.size / 1024 / 1024 < 3
      if (!isJPG) {
        this.$message.error('只能是图片!')
        this.$refs.upload.clearFiles();
        return false;
      }
      if(!isLT3M) {
        this.$message({
          message:'图片大小不能超过3M，请重新选择!',
          type: 'warning'
        });
        return false;
      }
      this.loading = this.$loading({
        lock: true,
        text: "上传中",
        background: "rgba(0, 0, 0, 0.7)",
      });
    },
    handleUploadError() {
      this.$message({
        type: "error",
        message: "上传失败",
      });
      this.loading.close();
    },
    cancel(){
      this.handleClose();
    },
    reset() {
      this.form={
        type:2,
        title:'',
        content:'',
        banner:'',
        description:''
      }
      this.srcList=[];
      // this.resetForm("form");
    },
    handleClose(){
      this.$confirm('是否确认关闭文章发布弹框?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then( ()=> {
        this.$emit('update:showNewsInfo',false);
        this.$emit('update:isEdit',false);
        this.reset();
      })
    },
  },
  mounted() {
  }
}
</script>

<style scoped>

</style>
