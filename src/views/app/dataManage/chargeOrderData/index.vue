  ...
  <div class="app-container">
    <el-form :model="queryParams"
      ...
      <el-form-item label="数据类型" prop="dataType">
        <el-select v-model="queryParams.dataType" placeholder="请选择数据类型" style="width: 100%;">
          <el-option label="所有订单" value="1"></el-option>
          <el-option label="预约成功订单" value="2"></el-option>
          <el-option label="预约失败订单" value="3"></el-option>
          <el-option label="换电成功订单" value="4"></el-option>
          <el-option label="换电失败订单" value="5"></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      legend:['预约成功订单','预约失败订单','换电成功订单','换电失败订单'],
      ...
      lineChartObJ:null,
      lineColors:['#7cb5ec','#434348','#90ed7d','#f7a35c'],
      seriesData:[],
      dateTypeData:[
        {
          type:'day',
  ...
  watch:{
    sidebar:{
      deep: true,
        handler(val) {
        console.log(this.lineChartObJ)
          setTimeout(()=>{
            this.lineChartObJ.resize();
          },50)
      }
    }
  },
  methods: {
    lineChart(id){
      if(this.lineChartObJ){
        this.lineChartObJ.dispose();
        this.lineChartObJ = null
      }
      this.lineChartObJ=echarts.init(document.getElementById(id));
      this.seriesData=[];
      ...
      this.lineChartObJ.setOption(option)
    },
    chartResize(){
      window.onresize=()=>{
        this.lineChartObJ.resize();
